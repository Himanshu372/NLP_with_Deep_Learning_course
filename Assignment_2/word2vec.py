...
def naiveSoftmaxLossAndGradient(
    ...
    obj_func = np.log(_softmax[outsideWordIdx])
    loss = np.negative(obj_func)
...
def negSamplingLossAndGradient(
    ...
    dot_two = np.negative(np.dot(neg_samples, centerWordVec))
    first_term = sigmoid(dot_one)
    second_term = sigmoid(dot_two)
    loss = -np.log(first_term) - np.sum(np.log(second_term))
    gradCenterVec = -(1 - sigmoid(dot_one)) * outsideVectors[outsideWordIdx] + np.sum(np.expand_dims((1 - sigmoid(dot_two)), axis = 1) * (neg_samples), axis = 0)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        l, dvc, duo = word2vecLossAndGradient(centerWordVectors[word2Ind[currentCenterWord]], word2Ind[word], outsideVectors, dataset)
        loss += l
        gradCenterVecs[word2Ind[currentCenterWord]] += dvc
        gradOutsideVectors += duo
